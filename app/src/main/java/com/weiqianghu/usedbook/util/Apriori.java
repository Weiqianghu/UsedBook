...
public class Apriori {
    ...
    public static void main(String[] args) throws Exception {
        ...
        trans.add(new ItemSet(new String[] { "I2", "I3" ,"I4"}));
        trans.add(new ItemSet(new String[] { "I1", "I2", "I4" }));
        trans.add(new ItemSet(new String[] { "I1", "I3" }));
        trans.add(new ItemSet(new String[] { "I2", "I3" }));
        trans.add(new ItemSet(new String[] { "I1", "I3" ,"I5"}));
        trans.add(new ItemSet(new String[] { "I1", "I2", "I3", "I5" }));
        trans.add(new ItemSet(new String[] { "I1", "I2", "I3" ,"I4"}));

        Map<Map<Set<String>, Set<String>>, Double> results=aprioriRecommend(trans);
        System.out.println("HUWEIQIANG");
        //System.out.println(results);

        for(Entry<Map<Set<String>, Set<String>>, Double> result : results.entrySet()){
            for(Entry<Set<String>, Set<String>> map : result.getKey().entrySet()){
                System.out.println(map.getKey()+"=>"+map.getValue()+":"+result.getValue());
            }
        }
    }

    public static Map<Map<Set<String>, Set<String>>, Double> aprioriRecommend(
            List<Set<String>> trans) {
        ...
        Map<Map<ItemSet, ItemSet>, Double> associateRules = associaeRules(rst);

        Map<Map<Set<String>, Set<String>>, Double> result = new HashMap<Map<Set<String>, Set<String>>, Double>();

        for (Map<ItemSet, ItemSet> associateRule : associateRules.keySet()) {

            Map<Set<String>, Set<String>> map = new HashMap<Set<String>, Set<String>>();

            for (Entry<ItemSet, ItemSet> mapEntry : associateRule.entrySet()) {
                Set<String> keySet = new TreeSet<String>(mapEntry.getKey());
                Set<String> valueSet = new TreeSet<String>(mapEntry.getValue());
                map.put(keySet, valueSet);
            }

            result.put(map, associateRules.get(associateRule));
        }

        return result;
    }

    private static Map<Map<ItemSet, ItemSet>, Double> associaeRules(
            Map<Integer, Set<ItemSet>> rst) {
        Map<ItemSet, ItemSet> directMap = new HashMap<ItemSet, ItemSet>();

        Map<Map<ItemSet, ItemSet>, Double> result = new HashMap<Map<ItemSet, ItemSet>, Double>();
            ...
            for (ItemSet set : entry.getValue()) {
                ...
                for (ItemSet subSet : subSets) {
                    ...
                    if (cnt1 / cnt2 >= MCONF) {
                        ...
                        Map<ItemSet, ItemSet> map = new HashMap<ItemSet, ItemSet>();
                        map.put(subSet, remainSet);
                        result.put(map, conf);
                    }
                }
            }
