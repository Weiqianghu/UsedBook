...
public class OrderExpressFragment extends BaseFragment implements IRecycleViewItemClickListener {
    ...
    private void gotoOrderExpressDetailFragment(int postion) {
        ...
        Fragment mFragment = mFragmentManager.findFragmentByTag(OrderExpressDetailFragment.TAG);
        if (mFragment == null) {
            mFragment = new OrderExpressDetailFragment();
        }
        Bundle bundle = new Bundle();
        bundle.putParcelable(Constant.DATA, mData.get(postion));
        mFragment.setArguments(bundle);
        Fragment from = mFragmentManager.findFragmentByTag(OrderFormFragment.TAG);
        if (null == from) {
            from = mFragmentManager.findFragmentByTag(OrderFormFragment.TAG);
        }

        FragmentUtil.switchContentAddToBackStack(from, mFragment, R.id.main_container, mFragmentManager, OrderExpressDetailFragment.TAG);
    }
